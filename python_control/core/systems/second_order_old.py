...
class SecondOrderSystem(TransferFunction):
    ...
    def __init__(self, a: float, b: float, name: str = ''):
        ...
        # we assume steady state at t_inf = 100 x T_nat
        self.t_inf = self.natural_period * 1.e2
    ...
    def damped_frequency(self) -> float:
        ...
        if self.damping_ratio <= 1.0:
            return self.natural_frequency * math.sqrt(1 - self.damping_ratio ** 2)
        else:
            return float('nan')
    ...
    def damping_ratio(self) -> float:
        ...
        """
        return (self.a / 2) / self.natural_frequency
    ...
    def get_natural_response_type(self) -> NaturalResponseType:
        ...
        elif 0.0 < self.damping_ratio < 1.0:
    ...
    def settling_time(self) -> float:
        ...
        if response_type != NaturalResponseType.UNDAMPED:
            if response_type != NaturalResponseType.OVERDAMPED and self.damping_ratio < 1:
                n = -math.log(0.02 * math.sqrt(1 - self.damping_ratio ** 2))
                d = self.damping_ratio * self.natural_frequency
                Ts = n / d
                return Ts
            else:
                sso = self.steady_state_output
    ...
    def peak_time(self) -> float:
        ...
        if self.get_natural_response_type() != NaturalResponseType.OVERDAMPED:
            try:
                Tp = math.pi / self.damped_frequency
    ...
    def percent_overshoot(self) -> float:
        ...
        if self.get_natural_response_type() == NaturalResponseType.UNDERDAMPED:
            a = self.damping_ratio * math.pi / math.sqrt(1 - self.damping_ratio ** 2)
    ...
    def characteristics(self) -> str:
        """Returns the transient response characteristics of the second-order
        system.
        """
        return (
            f"type of response: {self.get_natural_response_type().value}\n"
            f"natural frequency = {self.natural_frequency:.3f} rad/s\n"
            f"damping ratio = {self.damping_ratio:.3f}\n"
            f"percent overshoot = {self.percent_overshoot:.3f} %\n"
            f"rise time = {self.rise_time:.3f} s\n"
